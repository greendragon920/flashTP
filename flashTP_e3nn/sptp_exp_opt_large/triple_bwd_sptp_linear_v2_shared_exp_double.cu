...
__global__ void triple_bwd_sptp_lienar_kernel_v2_shared_exp(
    const double* __restrict__ mem_dT_dGin1,  // tx, from grad_output
    const double* __restrict__ mem_dT_dGin2,  // ty, from grad_output
    const double* __restrict__ mem_dT_dGW,    // t_theta, from grad_output
    const double* __restrict__ mem_dT_dGLo,   // tL, from grad_output
    
    const double* __restrict__ mem_dL_dO,  // Lo, from ctx
    const double* __restrict__ mem_dG_dLx, // gx, from ctx
    const double* __restrict__ mem_dG_dLy, // gy, from ctx
    const double* __restrict__ mem_dG_dLW, // g_theta, from ctx
    ...
    double* __restrict__ mem_dT_din1,
    double* __restrict__ mem_dT_din2,
    double* __restrict__ mem_dT_dW,
    double* __restrict__ mem_dT_dLo,
    double* __restrict__ mem_dT_dgx,
    double* __restrict__ mem_dT_dgy,
    double* __restrict__ mem_dT_dgW,
    ...
    for(int path_idx=path_idx_start; path_idx < path_idx_end; path_idx++){
        ...
        const int weight_pos = g_t_w_start + per_path_weight_pos[path_idx]+ WARPSIZE*channel_chunk_idx + threadIdx.x;
        double reg_w_path_norm = weight[weight_pos] * path_weight[path_idx];  // w pw
        double reg_F_w_path_norm = mem_dG_dLW[weight_pos] * path_weight[path_idx];  // gw pw
        double reg_T_w_path_norm = mem_dT_dGW[weight_pos] * path_weight[path_idx];  // tw pw
        ...
        for(ushort fiber_idx = fiber_idx_info.x; fiber_idx < fiber_idx_info.y; fiber_idx++){
            ...
            double dL_dOuter = common_dE_dO_deriv * reg_F_w_path_norm;  // bijk
            double dE_dOuter = common_dE_dO_deriv * reg_w_path_norm;    // aijk
            double dT_dOuter = common_dE_dO_deriv * reg_T_w_path_norm;  // cijk

            double common_dT_dGLo_deriv = my_shmem_dT_dGLo[fiber.z] * unique_cg_val[fiber.w];
            double dT_dGLo_W = common_dT_dGLo_deriv * reg_w_path_norm;   // dijk
            double dT_dGLo_gW = common_dT_dGLo_deriv * reg_F_w_path_norm;   // eijk
            double dT_dOuter_plus_dT_dGLo_W = dT_dOuter + dT_dGLo_W;     // cijk + dijk
   ...
   in1_idx = src_idx*in1_size + in1_start+threadIdx.x;
    ...
    // warp shuffle reduce
    for (int i = 0; i < in2_size; i++) {
        double sum_dT_din2 = my_shmem_dT_din2[i];
        double sum_dT_dgy = my_shmem_dT_dgy[i];
    ...
    const int g_dT_din2 = global_t_batch_idx*path_cnt*in2_size + exec_idx*in2_size;
...
void triple_bwd_sptp_linear_cuda_v2_shared_exp_double(
    ...
    const int shared_memory_bytes = sizeof(double) * per_block_batch * (WARPSIZE * (max_ir_dim*12+perwarp_in2_size*2) + in2_size*3);
    ...
    CHECK_CUDA_ERROR(cudaFuncSetAttribute(
        triple_bwd_sptp_lienar_kernel_v2_shared_exp<double>,
        cudaFuncAttributeMaxDynamicSharedMemorySize, shared_memory_bytes));

    triple_bwd_sptp_lienar_kernel_v2_shared_exp<double><<<grid, block, shared_memory_bytes>>>(
        mem_dT_dGin1.data<double>(),
        mem_dT_dGin2.data<double>(),
        mem_dT_dGW.data<double>(),
        mem_dT_dGLo.data<double>(),

        mem_dL_dO.data<double>(),
        mem_dG_dLx.data<double>(),
        mem_dG_dLy.data<double>(),
        mem_dG_dLW.data<double>(),
        ...
        mem_dT_din1.data<double>(),
        mem_dT_din2.data<double>(),
        mem_dT_dW.data<double>(),
        mem_dT_dLo.data<double>(),
        mem_dT_dgx.data<double>(),
        mem_dT_dgy.data<double>(),
        mem_dT_dgW.data<double>(),
